...
const PaymentManagement = () => {
  ...
  const [errors, setErrors] = useState({}){ useState } from 'react'
import { Search, User, DollarSign, CreditCard, Printer, CheckCircle, AlertCircle, Receipt, Download } from 'lucide-react'
...
const PaymentManagement = () => {
  ...
  const handleUserSearch = async (userId) => {
    if (!userId.trim()) {
      setErrors({ user: 'Please enter a valid User ID' })
      return
    }

    setUserLoading(true)
    try {
      // Get user fines using the correct librarian endpoint
      const response = await axios.get(`/librarian/fines/user/${userId}`)
  ...
  const handlePayment = async (e) => {
    ...
    try {
      const paymentPromises = selectedFines.map(fine => 
        axios.post(`/librarian/fines/${fine.id}/pay`)
      )
  ...
  const handleGenerateReceiptNoPayment = async () => {
    ...
    try {
      const response = await axios.post('/librarian/fines/generate-receipt', {
          ...
          <div className="form-card">
            ...
            <form onSubmit={handlePayment}>
              ...
              <div className="form-group">
                <label htmlFor="userId">Student ID / Roll Number</label>
                <div className="search-input-group">
                  ...
                  <button
                    ...
                    onClick={() => handleUserSearch(formData.userId)}
              ...
              <div className="user-header">
                ...
                <div className="user-status">
                  <span className="status-badge pending">
                    {userInfo.fines.length} Pending Fine{userInfo.fines.length !== 1 ? 's' : ''}
                  </span>
                </div>
              </div>
              ...
              {userInfo.fines.length > 0 ? (
                <div className="fines-list">
                  ...
                  {userInfo.fines.map((fine) => (
                    <div
                      key={fine.id}
                      className={`fine-item ${selectedFines.some(f => f.id === fine.id) ? 'selected' : ''}`}
                    ...
                    >
                      ...
                      <div className="fine-details">
                        ...
                        <div className="fine-meta">
                          <span>Created: {new Date(fine.created_date).toLocaleDateString()}</span>
                  ...
                  {errors.fines && <div className="error-message">{errors.fines}</div>}
                </div>
              ) : (
